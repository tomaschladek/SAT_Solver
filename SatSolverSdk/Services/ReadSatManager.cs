...
namespace SatSolverSdk.Services
{
    public class ReadSatManager : IReadSatManager
    {
        ...
        private SatDefinitionDto ProcessLine(string line, SatDefinitionDto definition, string fileName)
        {
            ...
            {
                definition.Weights = line.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries).Skip(1).Select(int.Parse).ToList();
            ...
            {
                var properties = line.Split(new []{' '}, StringSplitOptions.RemoveEmptyEntries);
            ...
            var variables = line.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries).Select(int.Parse).ToList();
